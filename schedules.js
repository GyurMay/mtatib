...
for(i=0; i<jr['groups'].length; i++){ //loop over each route (train Lines)
    ...
    for(j=0; j<numberOfTrains; j++){ //get arrival times rows
      ...
      tr.innerHTML += (`<td style='font-size:60%;'>${((new Date()).getHours() > d.getHours() + 24) ? d.getMinutes() - (new Date()).getMinutes() : d.getMinutes() - (new Date()).getMinutes() + 60} ${arrivalTimes[j]}</td>`);
    }
